<script lang="ts">
	...
	let alerts = $state<Array<{id: string; type: string; category: string; triggerTime: string; message: string}>>([]);

	// Mock data for now
	onMount(() => {
		alerts = [
			{ id: '1', type: 'SMS', category: 'Rituals', triggerTime: '06:00', message: 'Sandhyavandanam time' },
			{ id: '2', type: 'Alarm', category: 'Duties', triggerTime: '17:00', message: 'Homework reminder' }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">Alerts & Reminders</h1>
	...
	{#if activeTab === 'dashboard'}
		<div class="glass-card p-6 rounded-xl">
			<h2 class="text-xl font-semibold mb-4">Alerts Dashboard</h2>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				...
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Email Alerts</h3>
					<p class="text-2xl font-bold text-green-600">4</p>
				</div>
				<div class="bg-orange-100 p-4 rounded-lg">
					<h3 class="font-semibold text-orange-800">Alarms</h3>
	...
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Alert</button>
			<div class="space-y-2">
				{#each alerts as alert}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{alert.category} - {alert.triggerTime}</h3>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-4">
				<div class="bg-white/50 p-4 rounded-lg">
					<p class="font-semibold">System: Alert system updated with new rituals.</p>
					<p class="text-sm text-gray-600">2 days ago</p>
				</div>
				<div class="bg-white/50 p-4 rounded-lg">
					<p class="font-semibold">Grandparent: The morning reminders are very helpful.</p>
			...
			<div class="space-y-2">
				{#each alerts as alert}
					<div class="bg-yellow-100 p-4 rounded-lg border-l-4 border-yellow-500">
						<h3 class="font-semibold text-yellow-800">{alert.type}: {alert.category}</h3>
						<p class="text-sm">{alert.message} at {alert.triggerTime}</p>
					</div>
				{/each}
			</div>
		</div>
