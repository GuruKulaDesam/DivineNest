<script lang="ts">
	...
	let assets = $state<Array<{id: string; category: string; subCategory: string; name: string; location: string}>>([]);

	// Mock data for now
	onMount(() => {
		assets = [
			{ id: '1', category: 'Property', subCategory: 'House', name: 'Family Home', location: 'Kovaipudur' },
			{ id: '2', category: 'Vehicle', subCategory: 'Car', name: 'Toyota Innova', location: 'Garage' }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">Assets</h1>
	...
	{#if activeTab === 'dashboard'}
		<div class="glass-card p-6 rounded-xl">
			<h2 class="text-xl font-semibold mb-4">Assets Dashboard</h2>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				...
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Valuables</h3>
					<p class="text-2xl font-bold text-green-600">8</p>
				</div>
				<div class="bg-orange-100 p-4 rounded-lg">
					<h3 class="font-semibold text-orange-800">Documents</h3>
					<p class="text-2xl font-bold text-orange-600">16</p>
				</div>
			</div>
		</div>
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Asset</button>
			<div class="space-y-2">
				{#each assets as asset}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{asset.name} - {asset.category}</h3>
							<p class="text-sm text-gray-600">Sub-category: {asset.subCategory} | Location: {asset.location}</p>
						</div>
						<div class="space-x-2">
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-4">
				<div class="bg-white/50 p-4 rounded-lg">
					...
					<p class="text-sm text-gray-600 mb-2">Started by Father • 3 days ago</p>
					<p>How should we organize our important documents?</p>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-2">
				<div class="bg-yellow-100 border-l-4 border-yellow-500 p-4">
					<h3 class="font-semibold text-yellow-800">Document Expiry</h3>
					<p class="text-sm text-yellow-700">Car insurance expires in 15 days</p>
				</div>
			</div>
		</div>
	{/if}
