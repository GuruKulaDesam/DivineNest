<script lang="ts">
	...
	let prompts = $state<Array<{id: string; type: string; category: string; purpose: string; triggerTime: string}>>([]);

	// Mock data for now
	onMount(() => {
		prompts = [
			{ id: '1', type: 'Morning', category: 'Motivation', purpose: 'Daily inspiration', triggerTime: '07:00' },
			{ id: '2', type: 'Evening', category: 'Reflection', purpose: 'Daily review', triggerTime: '20:00' }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">AI Prompts</h1>
	...
	{#if activeTab === 'dashboard'}
		<div class="glass-card p-6 rounded-xl">
			<h2 class="text-xl font-semibold mb-4">AI Prompts Dashboard</h2>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div class="bg-blue-100 p-4 rounded-lg">
					<h3 class="font-semibold text-blue-800">Active Prompts</h3>
					<p class="text-2xl font-bold text-blue-600">12</p>
				</div>
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Morning</h3>
					<p class="text-2xl font-bold text-green-600">6</p>
				</div>
				<div class="bg-purple-100 p-4 rounded-lg">
					<h3 class="font-semibold text-purple-800">Evening</h3>
	...
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Prompt</button>
			<div class="space-y-2">
				{#each prompts as prompt}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{prompt.category} - {prompt.type}</h3>
							<p class="text-sm text-gray-600">Purpose: {prompt.purpose} | Time: {prompt.triggerTime}</p>
						</div>
						<div class="space-x-2">
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-4">
				<div class="bg-white/50 p-4 rounded-lg">
					<h3 class="font-semibold">AI Prompt Ideas</h3>
					<p class="text-sm text-gray-600 mb-2">Started by Admin • 1 day ago</p>
					<p>What prompts would be helpful for family motivation?</p>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-2">
				<div class="bg-green-100 border-l-4 border-green-500 p-4">
					<h3 class="font-semibold text-green-800">Prompt Executed</h3>
					<p class="text-sm text-green-700">Morning motivation prompt sent successfully</p>
				</div>
			</div>
		</div>
	{/if}
