<script lang="ts">
	...
	let rewards = $state<Array<{id: string; type: string; reason: string; points: number; person: string}>>([]);

	// Mock data for now
	onMount(() => {
		rewards = [
			{ id: '1', type: 'Star', reason: 'Completed homework', points: 10, person: 'Arjun' },
			{ id: '2', type: 'Karma', reason: 'Helped mother', points: 15, person: 'Ananya' }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">Rewards</h1>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div class="bg-yellow-100 p-4 rounded-lg">
					<h3 class="font-semibold text-yellow-800">Star Points</h3>
					<p class="text-2xl font-bold text-yellow-600">245</p>
				</div>
				<div class="bg-purple-100 p-4 rounded-lg">
					<h3 class="font-semibold text-purple-800">Karma Points</h3>
					<p class="text-2xl font-bold text-purple-600">180</p>
				</div>
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Total Rewards</h3>
					<p class="text-2xl font-bold text-green-600">425</p>
				</div>
			</div>
		</div>
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Reward</button>
			<div class="space-y-2">
				{#each rewards as reward}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{reward.reason} - {reward.type}</h3>
							<p class="text-sm text-gray-600">Points: {reward.points} | Person: {reward.person}</p>
						</div>
						<div class="space-x-2">
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-2">
				<div class="bg-green-100 border-l-4 border-green-500 p-4">
					<h3 class="font-semibold text-green-800">Reward Earned</h3>
					<p class="text-sm text-green-700">Arjun earned 10 star points for completing homework</p>
				</div>
			</div>
		</div>
	{/if}
