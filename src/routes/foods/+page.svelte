<script lang="ts">
	...
	let foods = $state<Array<{id: string; type: string; category: string; healthFactor: string; cost: number}>>([]);

	// Mock data for now
	onMount(() => {
		foods = [
			{ id: '1', type: 'South Indian', category: 'Breakfast', healthFactor: 'High', cost: 50 },
			{ id: '2', type: 'North Indian', category: 'Lunch', healthFactor: 'Medium', cost: 120 }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">Foods</h1>
	...
	{#if activeTab === 'dashboard'}
		<div class="glass-card p-6 rounded-xl">
			<h2 class="text-xl font-semibold mb-4">Foods Dashboard</h2>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div class="bg-orange-100 p-4 rounded-lg">
					<h3 class="font-semibold text-orange-800">South Indian</h3>
					<p class="text-2xl font-bold text-orange-600">25</p>
				</div>
				<div class="bg-blue-100 p-4 rounded-lg">
					<h3 class="font-semibold text-blue-800">North Indian</h3>
					<p class="text-2xl font-bold text-blue-600">18</p>
				</div>
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Healthy Options</h3>
	...
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Food</button>
			<div class="space-y-2">
				{#each foods as food}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{food.category} - {food.type}</h3>
							<p class="text-sm text-gray-600">Health: {food.healthFactor} | Cost: ₹{food.cost}</p>
						</div>
						<div class="space-x-2">
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-4">
				<div class="bg-white/50 p-4 rounded-lg">
					<h3 class="font-semibold">Recipe Sharing</h3>
					<p class="text-sm text-gray-600 mb-2">Started by Mother • 3 hours ago</p>
					<p>What's your favorite traditional recipe?</p>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-2">
				<div class="bg-yellow-100 border-l-4 border-yellow-500 p-4">
					<h3 class="font-semibold text-yellow-800">Expiry Alert</h3>
					<p class="text-sm text-yellow-700">Milk expires tomorrow</p>
				</div>
			</div>
		</div>
	{/if}
