<script lang="ts">
	...
	let activities = $state<Array<{id: string; type: string; category: string; frequency: string; status: string}>>([]);

	// Mock data for now
	onMount(() => {
		activities = [
			{ id: '1', type: 'Physical', category: 'Exercise', frequency: 'Daily', status: 'Active' },
			{ id: '2', type: 'Mental', category: 'Reading', frequency: 'Weekly', status: 'Active' },
			{ id: '3', type: 'Soulful', category: 'Meditation', frequency: 'Daily', status: 'Active' }
		];
	});
...
<div class="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 pb-20">
	<h1 class="text-2xl font-bold mb-6 text-gray-800">Activities</h1>
	...
	{#if activeTab === 'dashboard'}
		<div class="glass-card p-6 rounded-xl">
			<h2 class="text-xl font-semibold mb-4">Activities Dashboard</h2>
			<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
				<div class="bg-blue-100 p-4 rounded-lg">
					<h3 class="font-semibold text-blue-800">Physical</h3>
					<p class="text-2xl font-bold text-blue-600">8</p>
				</div>
				<div class="bg-green-100 p-4 rounded-lg">
					<h3 class="font-semibold text-green-800">Mental</h3>
					<p class="text-2xl font-bold text-green-600">12</p>
				</div>
				<div class="bg-purple-100 p-4 rounded-lg">
					<h3 class="font-semibold text-purple-800">Soulful</h3>
	...
	{:else if activeTab === 'crud'}
		<div class="glass-card p-6 rounded-xl">
			...
			<button class="bg-blue-500 text-white px-4 py-2 rounded-lg mb-4 hover:bg-blue-600">Add New Activity</button>
			<div class="space-y-2">
				{#each activities as activity}
					<div class="bg-white/50 p-4 rounded-lg flex justify-between items-center">
						<div>
							<h3 class="font-semibold">{activity.category} - {activity.type}</h3>
							<p class="text-sm text-gray-600">Frequency: {activity.frequency} | Status: {activity.status}</p>
						</div>
						<div class="space-x-2">
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-4">
				<div class="bg-white/50 p-4 rounded-lg">
					<h3 class="font-semibold">Family Activity Ideas</h3>
					<p class="text-sm text-gray-600 mb-2">Started by Mother • 2 hours ago</p>
					<p>What activities should we plan for the weekend?</p>
					<div class="mt-2 flex space-x-2">
						<button class="bg-blue-500 text-white px-3 py-1 rounded text-sm">Reply</button>
						<span class="text-sm text-gray-500">3 replies</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="glass-card p-6 rounded-xl">
			...
			<div class="space-y-2">
				<div class="bg-yellow-100 border-l-4 border-yellow-500 p-4">
					<h3 class="font-semibold text-yellow-800">Activity Reminder</h3>
					<p class="text-sm text-yellow-700">Time for evening meditation</p>
				</div>
			</div>
		</div>
	{/if}
